...
public class BombGuy : MonoBehaviour
{
    ...
    Animator anim;
    private Coroutine coroutine;//코루틴을 변수로 선언한 이유 - StopCoroutine를 쓰기 위해서
    ...
    private void Awake()
    {
        ...
        //coroutine = StartCoroutine(CoMove(() => { Debug.Log("코루틴 종료"); })); //StopCoroutine을 쓰기 전 어떤 코루틴을 멈출 것인지 지정한다.
